...
window.addEventListener("click", (event) => {
    ...
    async function fetchImagesOrder(eventName) {
        try {
            ...
            console.log('fetchImagesOrder執行過了')
    ...
    if (isMobile) {
        ...
        newWrap.innerHTML = detailsImageForMobile.map((detail, index) => `
            <a class="detail-box" 
            href="#" 
            data-name="${detail.name}" 
            data-index="${index}">
                <h2>${detail.name}</h2>
                <img src="${imagesForDiv[index]}" alt="${detail.name}" class="detail-image" />
            </a>
        `).join("");
        ...
        style.innerHTML = `
            ...
            @media (max-width: 768px) {
                .detail-box {
                    flex: 1 1 calc(100% - 20px); /* 小螢幕時，每個 box 佔 100% 寬度 */
...
async function setImageBlobs(eventData) {
    // 遍歷 eventData.additionalImages，生成 Blob 並設置圖片
    for (const [index, img] of eventData.additionalImages.entries()) {
        const imgElement = document.getElementById(`img-${eventData.folderName}-${index}`);
        try {
            const response = await fetch(img.path); // 請求圖片路徑
            if (!response.ok) throw new Error(`Failed to fetch image: ${img.path}`);

            const blob = await response.blob(); // 將圖片轉為 Blob
            const blobUrl = URL.createObjectURL(blob); // 生成 Blob URL

            imgElement.src = blobUrl; // 設置圖片 src
        } catch (error) {
            console.error(`Error loading image: ${img.path}`, error);
            imgElement.alt = "Image not available"; // 提供替代文字
        }
    }
}
...
async function generateMobilePage(eventName, eventData) {
    const imageCardsHTML = await generateImageCards(eventData); // 使用剛剛的函數
    return `
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <style>
            ...
            .page-event { text-align: center; margin: 20px; }
            .cover .heading { font-size: 24px; font-weight: bold; margin-bottom: 20px; }
            .logo-image img { width: 100%; max-width: 400px; margin: 10px auto; display: block; }
            .image-description { color: #555; margin: 10px 0; }
