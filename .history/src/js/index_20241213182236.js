    ...
    (gltf) => {
        ...
        if ("ontouchstart" in window) {
            window.addEventListener('touchstart', (e) => {
                e.preventDefault(); // 防止默認的觸控行為（如滾動）

                // 記錄初始觸控位置
                startTouch = e.touches[0].clientY;
                startTouchX = e.touches[0].clientX;

                // 將觸控點轉換為 Raycaster 的螢幕座標
                mouse.x = (startTouchX / sizesCanvas.width) * 2 - 1;
                mouse.y = -(startTouch / sizesCanvas.height) * 2 + 1;
                ...
                if (intersects.length > 0) {
                    ...
                    console.log("Touched object:", clickedObject);

                    // 添加觸控交互效果，例如動畫或高亮
                    gsap.to(clickedObject.position, {
                        z: clickedObject.position.z + 1,
                        duration: 0.5,
                        ease: "power2.out",
                    });
                } else {
                    console.log("No object touched.");
                }
            }, false);
