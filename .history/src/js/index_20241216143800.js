...
window.addEventListener("click", (event) => {
    ...
    async function fetchImagesOrder() {
        try {
            const response = await fetch('/api/images-order');
    ...
    if (isMobile) {
        ...
        // 監聽每個 detail-box 點擊事件
        document.querySelectorAll(".detail-box").forEach((box) => {
            box.addEventListener("click", async (event) => {
                const eventName = box.dataset.name;
                console.log('eventName', eventName)
                const url = box.dataset.url; // 取得 data-url 的值
                const name = box.dataset.name;

                const imagesData = await fetchImagesOrder();
...
// Function: 生成 mobile-page.html 內容
function generateMobilePage(eventName, eventData) {
    const imagesHTML = eventData.additionalImages.map((img, index) => `
    ...
    <body>
        <header>
            <div class="the-harmony-logo">
                <img src="/public/thumbnails/icon-W.png" alt="Logo" />
            </div>
        </header>
        <main>
