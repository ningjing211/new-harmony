...
window.addEventListener("click", (event) => {
    ...
    if (isMobile) {
        ...
        document.querySelectorAll(".detail-box").forEach((link) => {
            ...
            link.href = createMobilePageURL(eventName); // 生成對應 URL
        });
...
function createMobilePageURL(eventName) {
    const pageContent = generateMobilePage(eventName);
        ...
        <a id="image-${eventData.folderName}-${index}" class="logo-image">
            <img src="${img.path}" 
                 onerror="this.parentElement.style.display='none'; 
                          document.getElementById('${eventData.folderName}-${index}-des').style.display='none'">
        </a>
