    ...
    section.addEventListener("touchmove", (e) => {
        ...
        if (Math.abs(deltaX) > Math.abs(deltaY)) {
            ...
            localLinks.forEach((link) => {
                ...
                if (rect.left <= currentX && rect.right >= currentX && rect.top <= currentY && rect.bottom >= currentY) {
                    link.classList.add("highlight"); // 加入高亮樣式
                } else {
                    link.classList.remove("highlight"); // 移除高亮樣式
                }
            });
        }
        });
