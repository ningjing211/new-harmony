...
async function loadImagesDiv(detailsImage) {
    try {
        // Fetch JSON file containing image paths
        const response = await fetch('/api/images-order');
        if (!response.ok) throw new Error('Failed to fetch imagesOrder.json');
        ...
        // 遍歷 JSON 資料並更新 detailsImage
        const loadedDetails = data.map((group, index) => ({
            name: group.folderName || `Image-${index}`,
            url: group.url || '',
            path: group.path || ''
        }));

        console.log("Loaded Images:", loadedDetails);
...
window.addEventListener("click", (event) => {
    ...
    if (isMobile) {
        ...
        const canvas = document.querySelector(".main-webgl");
        if (canvas) canvas.remove(); // 移除 canvas
    
        // 載入圖片資料
        const detailsImage = await loadImages();
        ...
        newWrap.innerHTML = detailsImage.map((detail, index) => `
            <div class="detail-box">
                <img src="${detail.path}" alt="${detail.name}" class="detail-image" />
