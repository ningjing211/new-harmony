...
window.addEventListener("click", (event) => {
    ...
    if (isMobile) {
        ...
        newWrap.addEventListener("click", (event) => {
            // 確保點擊的是含有 data-name 的 detail-box
            const targetBox = event.target.closest(".detail-box");
            if (targetBox && targetBox.dataset.name) {
                const dataName = targetBox.dataset.name; // 取得 data-name 的值
                console.log('手機點擊dataName', dataName);
                addCards(dataName); // 執行 addCards 並傳入 data-name
            }
        });
