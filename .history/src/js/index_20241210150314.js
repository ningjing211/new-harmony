...
function shouldExecuteAddCards(eventName) {
    const main = document.getElementById("player");
    const existingPageEvent = main.querySelector(".page-event");
    const headingElement = existingPageEvent?.querySelector(".heading");
    
    // 檢查以下條件是否滿足：
    // 1. handlePlane 已執行（videoLook === true 表示播放器打開）
    // 2. playerSource.src 已設置且非空
    // 3. existingPageEvent 已生成
    // 4. class="heading" 的值已正確設置為 eventName
    return (
        !videoLook && // 確保播放器未打開
        !playerSource.src && // 確保視頻源未設置
        (!existingPageEvent || headingElement?.innerText !== eventName) // 確保頁面未生成或活動名稱不一致
    );
}
