    ...
    if (isMobile) {
        ...
        document.querySelectorAll(".detail-box").forEach((box) => {
            box.addEventListener("click", async () => {
                const eventName = box.dataset.event;
                console.log("點擊的事件名稱:", eventName);

                // 呼叫 addCards()，傳入 eventName
                await addCards(eventName);

                // 重定向或執行其他邏輯（如跳轉頁面）
                window.location.href = box.dataset.url; // 可選擇性地重導向到 URL
        });
    });
