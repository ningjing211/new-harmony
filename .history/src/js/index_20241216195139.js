...
async function loadDetailsImage() {
    try {
        ...
        let data = await getImagesOrder(); // 使用單一 fetch
        data = data.reverse();
...
async function loadDetailsImageForMobile() {
    try {
        // Fetch JSON file containing image paths
        const data = await getImagesOrder(); // 使用單一 fetch
        
        // console.log("Fetched JSON data for detailsImage:", data);

        const loadDetailsImageForMobile = data.map(group => ({
            url: group.video?.url || 'URL not available', // 安全提取 video.url
            name: group.folderName || 'Name not available' // 安全提取 folderName
        }));
        ...
        // console.log('loadedDetailsImage', loadedDetailsImage);
        // console.log("images-inside:", loadedDetailsImage); // 確保這裡是資料處理完成後
        return loadDetailsImageForMobile; // 回傳已完成的 images 陣列
    } catch (error) {
        console.error("Error loading images:", error);
        return []; // 如果發生錯誤，回傳空陣列
    }
...
window.addEventListener("click", (event) => {
    ...
    async function fetchImagesOrder(eventName) {
        try {
            // 從後端獲取 JSON 資料
            const imagesData = await getImagesOrder(); // 使用單一 fetch
            console.log('執行過了')
    ...
    if (isMobile) {
        ...
        document.querySelectorAll(".detail-box").forEach((link) => {
            ...
            link.addEventListener("click", async (e) => {
                ...
                const eventData = await getImagesOrder(eventName); // 先獲取 eventData
                if (eventData) {
                    // 動態生成 Blob URL
...
async function addCards(eventName) {
    ...
    try {
        // 從後端獲取 JSON 資料
        let imagesData = await getImagesOrder(); // 使用單一 fetch
