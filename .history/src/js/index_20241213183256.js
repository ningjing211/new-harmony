...
for (let i = 0; i < 10; i++) {
    ...
    // Plane
    let plane = new THREE.Mesh(planeGeometry, planesMaterial[i])

    
    if (isMobile) {
        plane = new THREE.Mesh(new THREE.PlaneGeometry(2.5, 1.5), planesMaterial[i]); // 修改幾何範圍
...
const line = new THREE.Line(
    new THREE.BufferGeometry().setFromPoints([camera.position, currentIntersect.point]),
